...
const SlideInNotification = () => {
  ...
  const removeNotif = (id: number) => {
    setNotification((pv) => pv.filter((n) => n.id !== id));
  };

  return (
    <div className="bg-white min-h-screen flex items-center justify-center">
      <button
        className="text-sm text-white bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition-all font-medium px-4 py-2 rounded"
        onClick={() => setNotification((pv) => [generateRandomNotif(), ...pv])}
      >
        Add notification
      </button>
