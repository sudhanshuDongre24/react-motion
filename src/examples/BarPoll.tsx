...
const BarPoll = () => {
  ...
  return (
    <section className="bg-slate-900 px-4 py-12 min-h-screen flex items-center justify-center">
...
const Options = ({
  ...
  const handleIncrementVotes = (vote: VoteType) => {
      ...
      <div className="mb-6 space-y-2">
        {votes.map((vote) => (
          <motion.button
            ...
            onClick={() => handleIncrementVotes(vote)}
            key={vote.title}
        ...
        return (
          <div>
            <div className="relative flex h-full w-full items-end overflow-hidden rounded-2xl bg-gradient-to-b from-slate-700 to-slate-800">
              ...
              />
              <span className="absolute bottom-0 left-[50%] mt-2 inline-block w-full -translate-x-[50%] p-2 text-center  text-slate-50">
                <b>{vote.title}</b>
                <br></br>
